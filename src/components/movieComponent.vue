  ...
  <div class="movie-card flex items-center justify-center">
    <div v-show="!loading" class="absolute inset-0 -z-1 flex items-center justify-center">
      <Transition name="fade-glow">
        <img :key="movie.cover" :src="movie.cover" :alt="movie.title" draggable="false"
          class="w-full max-w-sm aspect-[2/3] object-cover  scale-110 pointer-events-none transition-all duration-1000 blur-background"
          aria-hidden="true" />
      </Transition>
    </div>
