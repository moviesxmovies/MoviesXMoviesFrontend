...
describe("FilmographyComponent", () => {
  ...
  describe("when a non-empty list is provided", () => {
    ...
    it("renders the AccordionPanel for the list", () => {
      ...
      expect(wrapper.find("[data-testid='AccordionPanel']").exists()).toBe(true);
    });
  ...
  describe("sentinel ref emit", () => {
    ...
    it("emits the element as first argument", async () => {
      ...
      const [firstArg] = emitted![emitted!.length - 1];
      // Could be an HTMLElement or null depending on env
      expect(firstArg === null || firstArg instanceof HTMLElement || firstArg !== undefined).toBe(true);
    });
  });

  // ── index prop ───────────────────────────────────────────────────────────
  describe("index prop", () => {
    it("passes the index value to AccordionPanel", () => {
      const wrapper = mountComponent({ list: [makeMovie(1)], index: 5 });
      const panel = wrapper.find("[data-testid='AccordionPanel']");
      // The stub renders data-testid, not :value, but we can verify mounting
